		...
		ApplyStoreAction: func(tx Tx, sa *api.StoreAction) error {
			...
			case *api.StoreAction_Attachment:
				...
				case api.StoreActionKindUpdate:
					fmt.Printf("\nGot attachment update event, what do I do with it ?\n")
					return nil
				}
			}
...
type attachmentEntry struct {
	*api.ExecutorAttachment
}

func (a attachmentEntry) ID() string {
	return a.ExecutorAttachment.ID
}

func (a attachmentEntry) Meta() api.Meta {
	return a.ExecutorAttachment.Meta
}

func (a attachmentEntry) SetMeta(meta api.Meta) {
	a.ExecutorAttachment.Meta = meta
}

func (a attachmentEntry) Copy() Object {
	return attachmentEntry{a.ExecutorAttachment.Copy()}
}

func (a attachmentEntry) EventCreate() state.Event {
	return state.EventCreateAttachment{Attachment: a.ExecutorAttachment}
}

func (a attachmentEntry) EventUpdate() state.Event {
	return state.EventUpdateAttachment{Attachment: a.ExecutorAttachment}
}

func (a attachmentEntry) EventDelete() state.Event {
	return state.EventDeleteAttachment{Attachment: a.ExecutorAttachment}
}

// CreateAttachment adds a new attachment to the store.
// Returns ErrNameConflict if the ID is already taken.
func CreateAttachment(tx Tx, a *api.ExecutorAttachment) error {
	// Ensure the name is not already in use.
	if tx.lookup(attachmentTable, indexName, strings.ToLower(a.Spec.Annotations.Name)) != nil {
...
func UpdateAttachment(tx Tx, a *api.ExecutorAttachment) error {
	// Ensure the name is either not in use or already used by this same ExecutorAttachment.
	if existing := tx.lookup(attachmentTable, indexName, strings.ToLower(a.Spec.Annotations.Name)); existing != nil {
		if existing.ID() != a.ID {
			return ErrNameConflict
		}
	}
...
func GetAttachment(tx ReadTx, id string) *api.ExecutorAttachment {
	a := tx.get(attachmentTable, id)
	if a == nil {
		return nil
	}
	return a.(attachmentEntry).ExecutorAttachment
}

// FindAttachments selects a set of attachments and returns them.
func FindAttachments(tx ReadTx, by By) ([]*api.ExecutorAttachment, error) {
	...
	list := []*api.ExecutorAttachment{}
	appendResult := func(o Object) {
		list = append(list, o.(attachmentEntry).ExecutorAttachment)
	}
...
func (ai attachmentIndexerByID) FromObject(obj interface{}) (bool, []byte, error) {
	...
	val := a.ExecutorAttachment.ID + "\x00"
